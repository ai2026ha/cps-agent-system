...
async function renderGameItems(){
 ...
 $('#content').innerHTML=panel('道具库',`${importBox}${queryBox}<div class="query-scope-note">默认每页只加载 ${gameItemSearch.page_size} 条，避免大规模道具库打开时卡顿。</div>${table(rows,cols)}${pager}`,manage?`<button class="btn" id="importGameItemsBtn">⇧ 导入文件</button> <button class="btn danger-action" id="clearGameItemsBtn" ${total<=0?'disabled':''}>清空全部道具</button> <button class="btn primary" id="addGameItemBtn">＋ 新增道具</button>`:'');
