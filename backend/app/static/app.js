...
async function loadView(view){if(!canView(view)){currentView=firstAllowedView();return loadView(currentView)}if(view!=='dashboard')stopSystemMetricsPolling();const [t,s]=titles[view]||['',''];$('#pageTitle').textContent=t;$('#pageSub').textContent=s;$('#content').innerHTML='<div class="panel"><div class="empty">加载中...</div></div>';try{
 ...
 if(view==='gameItems') return renderGameItems();
...
async function renderPrivilegeCards(){
 ...
 $('#addPrivilegeBtn').onclick=()=>openPrivilegeCardForm().catch(e=>showToast(e.message,'error',4200));
}
...
async function renderGameItems(){
 ...
 const itemTable=`<div class="game-item-table-scroll">${table(rows,cols)}</div>`;
 $('#content').innerHTML=`<div class="game-item-shell">${panel('道具库',`${importBox}${queryBox}${dangerBox}<div class="query-scope-note">默认每页只加载 ${gameItemSearch.page_size} 条，避免大规模道具库打开时卡顿。</div>${itemTable}${pager}`,manage?`<button class="btn" id="importGameItemsBtn">⇧ 导入文件</button> <button class="btn primary" id="addGameItemBtn">＋ 新增道具</button>`:'')}</div>`;
 ...
 if(manage){
   ...
   const clearBtn=$('#clearGameItemsBtn');if(clearBtn)clearBtn.onclick=()=>clearAllGameItems(total);
